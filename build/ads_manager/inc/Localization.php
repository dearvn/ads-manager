...
use Dearvn\Ads_manager\base\UtilsProvider;
...
class Localization {
    ...
    protected function getPackageInfo($type) {
        if ($type === UtilsLocalization::$PACKAGE_INFO_BACKEND) {
            return [path_join(ADS_MANAGER_PATH, 'languages'), ADS_MANAGER_TD];
        } else {
            return [path_join(ADS_MANAGER_PATH, Assets::$PUBLIC_JSON_I18N), ADS_MANAGER_TD];
        }
    }
}
